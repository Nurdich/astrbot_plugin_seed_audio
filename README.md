...
若返回 `Seed Audio 插件运行正常`，说明插件已加载；若仍走 LLM 回复，请检查：

若仍走 LLM 流式回复，请确认已升级到 **v1.0.7+** 并重载插件；控制台应出现 `[seed-audio] on_llm_request 拦截指令`。

1. WebUI → **插件**：`astrbot_plugin_seed_audio` 无红色报错，状态为已启用
2. 目录必须是 `AstrBot/data/plugins/astrbot_plugin_seed_audio`（**不能**是 `seed-audio`）
3. WebUI → **控制台**：重载后应出现 `[seed-audio] 插件 v1.0.5 已加载`；若有 `ModuleNotFoundError`，手动安装 `httpx`
4. WebUI → **插件 → 管理行为 → 指令管理**：确认 `/seedtts`、`/seedaudio_ping` 未被禁用
5. 修改代码后 **重载插件**，必要时重启 AstrBot
